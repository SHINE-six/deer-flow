...
def test_build_run_config_preserves_explicit_user_id():
    from app.gateway.services import build_run_config
    ...
    assert config["configurable"]["user_id"] == "user-123"
...
def test_get_authenticated_user_id_reads_forwarded_header():
    from app.gateway.services import get_authenticated_user_id
    from starlette.requests import Request

    request = Request(
        {
            "type": "http",
            "headers": [(b"x-auth-user-id", b"user-42")],
        }
    )

    assert get_authenticated_user_id(request) == "user-42"


def test_get_authenticated_user_id_ignores_blank_header():
    from app.gateway.services import get_authenticated_user_id
    from starlette.requests import Request
