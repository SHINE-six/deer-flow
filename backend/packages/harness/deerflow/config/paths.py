...
_MOUNT_SOURCE_CACHE: dict[str, str | None] = {}
...
def _is_host_absolute(path: str) -> bool:
    """Return True for POSIX, UNC, and Windows drive-letter absolute paths."""
    return os.path.isabs(path) or bool(_WINDOWS_ABS_RE.match(path))


def discover_host_mount_source(container_path: str | Path) -> str | None:
    """Best-effort lookup of the host bind source for a mounted container path.

    In Docker-outside-of-Docker deployments the backend container may not know
    the real host checkout path. When the Docker CLI is available and the
    current container can inspect itself through the mounted Docker socket,
    resolve the host bind source by matching the mount destination.
    """
    target = str(Path(container_path))
    cached = _MOUNT_SOURCE_CACHE.get(target)
    if cached is not None or target in _MOUNT_SOURCE_CACHE:
        return cached

    container_id = os.getenv("HOSTNAME")
    if not container_id:
        _MOUNT_SOURCE_CACHE[target] = None
        return None
    ...
    try:
        ...
        for mount in mounts:
            if mount.get("Destination") == target:
                ...
                _MOUNT_SOURCE_CACHE[target] = source
                return source

        # Some container paths, such as /app/skills, are represented by
        # multiple nested bind mounts like /app/skills/public and
        # /app/skills/custom. Reconstruct the common host root if all nested
        # mounts imply the same parent directory.
        ...
        for mount in mounts:
            ...
            if not destination or not source:
                continue
            try:
                dest_posix = PurePosixPath(destination)
                relative = dest_posix.relative_to(target_posix)
            except Exception:
                continue
            ...
            elif inferred_root != candidate:
                inferred_root = None
                break

        if inferred_root is not None:
            _MOUNT_SOURCE_CACHE[target] = inferred_root
            return inferred_root
    except Exception as exc:
        logger.debug("Could not inspect current container mount for %s: %s", target, exc)
...
class Paths:
    ...
    def _host_base_dir_str(self) -> str:
        ...
        This value is passed to the **host** Docker daemon for bind-mount
        sources, so it must be an absolute path on the **host** filesystem.
        Set ``DEER_FLOW_HOST_BASE_DIR`` to an absolute host path in DooD
        (Docker-outside-of-Docker) deployments.
        """
        if env := os.getenv("DEER_FLOW_HOST_BASE_DIR"):
            if _is_host_absolute(env):
                if _WINDOWS_ABS_RE.match(env) or Path(env).exists():
                    return env
                logger.warning(
                    "DEER_FLOW_HOST_BASE_DIR does not exist on the host (%r); "
                    "trying to discover the bind source for %s.",
                    env,
                    self.base_dir,
                )
            else:
                logger.warning(
                    "DEER_FLOW_HOST_BASE_DIR is not an absolute host path (%r); "
                    "trying to discover the bind source for %s.",
                    env,
                    self.base_dir,
                )

        if discovered := discover_host_mount_source(self.base_dir):
            return discovered
