...
from deerflow.config.paths import VIRTUAL_PATH_PREFIX, discover_host_mount_source, get_paths
...
class AioSandboxProvider(SandboxProvider):
    ...
    @staticmethod
    def _resolve_host_skills_path(skills_path: Path) -> str | None:
        """Resolve a host-visible skills path for Docker bind mounts.

        In native/local execution, ``skills_path`` already points at a usable host
        path. In Docker-outside-of-Docker deployments, the gateway/langgraph
        container may see skills at an internal path like ``/app/skills`` while
        the host Docker daemon needs the real host checkout path instead.
        """
        env_path = os.environ.get("DEER_FLOW_HOST_SKILLS_PATH")
        if env_path:
            if not os.path.isabs(env_path):
                logger.warning(
                    f"DEER_FLOW_HOST_SKILLS_PATH is relative ({env_path!r}); "
                    "ignoring it and trying to infer an absolute host path."
                )
            elif Path(env_path).exists():
                return env_path
            else:
                logger.warning(
                    f"DEER_FLOW_HOST_SKILLS_PATH does not exist on the host ({env_path!r}); "
                    "trying fallback locations derived from DEER_FLOW_HOST_BASE_DIR."
                )

        discovered = discover_host_mount_source(skills_path)
        if discovered:
            return discovered

        host_base_dir = os.environ.get("DEER_FLOW_HOST_BASE_DIR")
        if host_base_dir:
            if not os.path.isabs(host_base_dir):
                logger.warning(
                    f"DEER_FLOW_HOST_BASE_DIR is relative ({host_base_dir!r}); "
                    "cannot infer a host skills path from it."
                )
            else:
                seen: set[Path] = set()
                for ancestor in [Path(host_base_dir), *Path(host_base_dir).parents]:
                    for candidate in (ancestor / "skills", ancestor / "deer-flow" / "skills"):
    ...
    def _get_skills_mount() -> tuple[str, str, bool] | None:
        ...
        try:
            ...
            host_skills = AioSandboxProvider._resolve_host_skills_path(skills_path)
            if host_skills:
