...
from deerflow.agents.middlewares.rag_scope_middleware import RagScopeMiddleware
from deerflow.agents.middlewares.rag_upload_path_middleware import RagUploadPathMiddleware
...
def _build_middlewares(config: RunnableConfig, model_name: str | None, agent_name: str | None = None, custom_middlewares: list[AgentMiddleware] | None = None):
    ...
    # Scope RAG tool calls to the authenticated user for custom agents.
    middlewares.append(
        RagScopeMiddleware(
    ...
    middlewares.append(RagUploadPathMiddleware())
