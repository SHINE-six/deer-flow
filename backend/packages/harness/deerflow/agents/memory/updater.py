...
_IDENTITY_FACT_PREFIX_NAME = "User's name is "
_IDENTITY_FACT_PREFIX_EMAIL = "User's email is "


def seed_user_identity_facts(
    user_name: str | None = None,
    user_email: str | None = None,
    agent_name: str | None = None,
) -> None:
    """Persist user identity (name/email) as high-confidence facts in memory.

    First call (no ``_identitySeeded`` flag in memory.json):
      - Creates facts for name and/or email with confidence 1.0, source "auth".
      - Sets ``_identitySeeded: true`` in memory.json so future calls from any
        device/browser know seeding has already happened.

    Subsequent calls (``_identitySeeded`` is already true):
      - Only *updates* existing facts if the value changed (e.g. display name
        was edited in the auth provider).
      - Does NOT re-create facts the user deleted from the settings page.

    This means deletion from the settings page is permanent: the fact will not
    come back on the next login from a different browser or device.
    The flag is cleared when the user clears all memory, at which point the
    next login seeds fresh facts.
    """
    ...
    try:
        memory_data = get_memory_data(agent_name)
        already_seeded: bool = bool(memory_data.get("_identitySeeded", False))
        facts = list(memory_data.get("facts", []))
        changed = False

        identity_pairs: list[tuple[str, str | None]] = [
            (_IDENTITY_FACT_PREFIX_NAME, user_name),
            (_IDENTITY_FACT_PREFIX_EMAIL, user_email),
        ]

        for prefix, value in identity_pairs:
            if not value:
                continue

            desired_content = f"{prefix}{value}"
            existing_fact = None
            for fact in facts:
            ...
            if existing_fact is not None:
                # Update in place only if the value changed.
                if existing_fact.get("content") != desired_content:
                    existing_fact["content"] = desired_content
                    changed = True
            elif not already_seeded:
                # First-time seeding: create the fact.
                facts.append(
                    {
                        "id": f"fact_{uuid.uuid4().hex[:8]}",
                ...
                changed = True
            # else: already_seeded=True and fact was deleted by user — respect deletion.

        if changed or not already_seeded:
            updated_memory = dict(memory_data)
            updated_memory["facts"] = facts
            updated_memory["_identitySeeded"] = True
            _save_memory_to_file(updated_memory, agent_name)
    except Exception:
        logger.debug("Failed to seed user identity facts", exc_info=True)
