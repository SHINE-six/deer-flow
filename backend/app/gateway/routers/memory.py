...
class SeedIdentityRequest(BaseModel):
    """Request model for seeding user identity into memory."""
...
class SeedIdentityResponse(BaseModel):
    """Response model for identity seeding."""

    seeded: bool = Field(..., description="Whether new identity facts were created or updated")


@router.post(
    "/memory/seed-identity",
    response_model=SeedIdentityResponse,
    summary="Seed User Identity",
    description="Persist user identity (name/email) as memory facts. Idempotent: skips if facts already exist with the same values.",
)
async def seed_identity(request: SeedIdentityRequest) -> SeedIdentityResponse:
    """Seed user identity from auth session into memory facts.

    Called once on first sign-in. Creates high-confidence context facts
    for the user's name and email. If facts already exist with the same
    values, this is a no-op. If values changed, existing facts are updated.
    """
    if not request.name and not request.email:
        return SeedIdentityResponse(seeded=False)

    try:
        seed_user_identity_facts(user_name=request.name, user_email=request.email)
        return SeedIdentityResponse(seeded=True)
    except Exception as exc:
        raise HTTPException(status_code=500, detail="Failed to seed user identity.") from exc
