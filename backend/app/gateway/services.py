...
def get_authenticated_user_id(request: Request) -> str | None:
    """Return the authenticated Better Auth user id forwarded by nginx."""
    user_id = request.headers.get(AUTH_USER_ID_HEADER)
    if user_id is None:
        return None
...
async def start_run(
    ...
    if authenticated_user_id is not None:
        config.setdefault("configurable", {}).setdefault("user_id", authenticated_user_id)
